CREATE OR REPLACE FUNCTION atomic_create_marketplace_order(
  ...
  BEGIN
    ...
    LOOP
      INSERT INTO orders (
        ...
        v_master_id,
        p_master_order->>'paymentId',
        NULLIF(v_sub_order->>'affiliate_id', '')::UUID,
      ...
      IF NULLIF(v_sub_order->>'affiliate_id', '') IS NOT NULL AND (v_sub_order->>'affiliate_commission')::NUMERIC > 0 THEN
        PERFORM increment_affiliate_earnings(NULLIF(v_sub_order->>'affiliate_id', '')::UUID, (v_sub_order->>'affiliate_commission')::NUMERIC);
      END IF;
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Rollback everything (Postgres handles this automatically in functions unless caught, but here we can just raise)
