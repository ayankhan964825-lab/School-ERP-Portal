...
CREATE TABLE IF NOT EXISTS wallet_transactions (
  ...
  type TEXT NOT NULL                            -- 'cod_credit' | 'prepaid_credit' | 'commission_debit' | 'shipping_debit' | 'payout' | 'adjustment'
    CHECK (type IN ('cod_credit', 'prepaid_credit', 'commission_debit', 'shipping_debit', 'payout', 'adjustment')),
...
-- 3.1: Also add is_god_mode just in case the table already existed before the migration update
...
CREATE POLICY "Public can view approved listings" ON marketplace_listings FOR SELECT USING (status = 'approved' AND is_active = true);
